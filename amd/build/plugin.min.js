define('tiny_imageia/plugin', [
  'editor_tiny/loader',
  'editor_tiny/utils',
  'tiny_imageia/common',
  'tiny_imageia/options',
  'tiny_imageia/configuration',
  'core/str',
  'core/ajax',
  'core/templates'
], function(Loader, Utils, Common, Options, Configuration, Str, Ajax, Templates) {
  const getTinyMCE = Loader.getTinyMCE;
  const getPluginMetadata = Utils.getPluginMetadata;
  const component = Common.component;
  const pluginName = Common.pluginName;
  const buttonName = Common.buttonName;
  const menuItemName = Common.menuItemName;
  const registerOptions = Options.register;
  const isConfigured = Options.isConfigured;
  const getContextId = Options.getContextId;
...
async function openImageIADialog(editor, configured) {
    ...
    async function generate() {
        ...
        try {
            ...
            errorDiv.innerHTML     = `<strong>${T.error_prefix}</strong> ${err.message}<br>
                                      <small style="color:#9ca3af;">${T.error_hint}</small>`;
...
const defaultExport = new Promise(async(resolve) => {
